...
xls = list.files(path,pattern = "yahoo") #找出在路徑 : path 中名稱有XX的檔案
...
training_split = 0.7
N = 50
perform_ALL33 <- data.frame(name = c("MSE", "Accuracy", "Macro-F1"))
...
for (j in 1:length(nxls)){
  ...
  perform_ALL33 = cbind(perform_ALL33, perform_DF)
...
data_num
count_Y
write.table(perform_ALL33, file="GModel_yahoo_2.csv", sep=",", row.names=F, col.names = T, na = "NA")
