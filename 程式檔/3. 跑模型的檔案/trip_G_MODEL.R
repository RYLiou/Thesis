...
path = paste("D:\\desktop\\論文\\data\\文字輸出", sep="") #設定讀檔路徑

xls = list.files(path,pattern = "trip") #找出在路徑 : path 中名稱有XX的檔案

#############################################################
nxls = xls[c(1,2,3,5,6,7,9,10,11,13,14,15,17,18,19)]  # 維度D = 50, 100, 150
nxls = xls[c(4,8,12,16,20,21)]  # 維度D = 200, 300
#############################################################
## 要調整157行
...
training_split = 0.7
N = 50
perform_ALL35 <- data.frame(name = c("MSE", "Accuracy", "Macro-F1"))
data_num <- data.frame(name = c("train", "test", "total"))
count_Y <- NULL

### run GModel
for (j in 1:length(nxls)){
  
  set.seed(2020)
  
  ### read data
  filename =  paste(nxls[j], sep="")
  ...
  count_Y2 = rbind(table(train$V1), table(test$V1), table(data_a$V1))
  count_Y = rbind.fill(count_Y, data.frame(count_Y2))
  
  perform_ALL35 = cbind(perform_ALL35, perform_DF)
...
data_num
count_Y
write.table(perform_ALL35, file="GModel_trip_2.csv", sep=",", row.names=F, col.names = T, na = "NA")


### assumption test
xls
filename =  paste(nxls2[6], sep="")
data <- read.table(filename, sep = ',')
data = data[-1,]


## transform to ordered
data[,1] <- as.ordered(data[,1])

## ol reg.
#ol1 <- clm(data$V1~., data = data)
logistic_model = multinom(formula, data = data)
## testing
