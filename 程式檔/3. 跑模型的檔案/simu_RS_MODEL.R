...
training_split = 0.7
N = 1
perform_ALL1_try <- data.frame(name = c("MSE", "Accuracy", "Macro-F1"))
...
for (j in 1:10){
  ...
  perform_ALL1_try = cbind(perform_ALL1_try, perform_DF)
  
}
perform_ALL1_2
data_num_RS
count_Y_RS
write.table(perform_ALL1_2, file="RSModel_simulation.csv", sep=",", row.names=F, col.names = T, na = "NA")
